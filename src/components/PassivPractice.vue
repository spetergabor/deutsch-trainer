<template>
  <div class="practice-container passiv-practice">
    <h1>Passiv</h1>

    <section v-if="!selectedMode" class="passiv-mode-picker">
      <button
        v-for="mode in practiceModes"
        :key="mode.id"
        class="passiv-mode-card"
    ...
    >
      ...
      <div v-if="currentQuestion && !showStatistics" class="quiz-area">
        <div class="question-card passiv-question-card">
          <span class="passiv-type">{{ currentQuestion.label }}</span>
          <p class="passiv-prompt">{{ currentQuestion.prompt }}</p>
          <h2 class="display-word passiv-sentence">
            {{ currentQuestion.sentence }}
          </h2>
        </div>

        <div v-if="selectedMode === 'choice'" class="passiv-options">
          <button
            v-for="option in currentQuestion.options"
            :key="option"
            class="passiv-option-btn"
        ...
        <div v-else class="input-wrapper ui-unit passiv-writing-input">
          <textarea
            ref="answerInput"
            v-model="userAnswer"
            class="passiv-answer-textarea"
            placeholder="Írd ide a Passiv mondatot..."
            :disabled="isAnswered"
            :class="{
              'input-correct': isAnswered && isCorrect,
              'input-wrong': isAnswered && isCorrect === false,
            }"
            @keydown.enter.exact.prevent="handleWritingEnter"
          ></textarea>
        </div>
...
import { passivQuestions, passivWritingQuestions } from "../data/passiv";
...
export default {
      ...
      practiceModes: [
        {
          ...
          description: "Alakok, igeidők, von/durch és Passiv felismerés.",
        },
        {
          id: "writing",
          icon: "✍️",
          title: "Átalakítás",
          description: "Aktiv mondatok átírása megadott Passiv időbe.",
        },
      ],
    };
  },
  ...
  methods: {
    ...
    selectMode(mode) {
      ...
      this.allQuestions =
        mode === "writing" ? passivWritingQuestions : passivQuestions;
      this.pickNewSet();
    },
    ...
    async saveResults() {
      try {
        await saveExerciseResult(
          this.selectedMode === "writing" ? "passiv-writing" : "passiv",
        ...
        console.error("Passiv eredmény mentési hiba:", error.message);
      }
    },
  },
};
