<template>
  <div class="practice-container sich-verben-practice">
    <h1>Sich-Verben</h1>
    ...
    >
      ...
      <div v-if="currentQuestion && !showStatistics" class="quiz-area">
        <div class="question-card sich-question-card">
          <span class="sich-infinitive">{{ currentQuestion.infinitive }}</span>
...
export default {
  ...
  methods: {
    async loadQuestions() {
      try {
        const databaseQuestions = await fetchActiveExerciseItems("sich-verben");
        this.allQuestions = [...sichVerbenData, ...databaseQuestions];
      } catch (error) {
        console.error("Sich-Verben feladatok betöltési hiba:", error.message);
        this.allQuestions = sichVerbenData;
      }
    ...
    async saveResults() {
      try {
        ...
        console.error("Sich-Verben eredmény mentési hiba:", error.message);
      }
    },
  },
};
